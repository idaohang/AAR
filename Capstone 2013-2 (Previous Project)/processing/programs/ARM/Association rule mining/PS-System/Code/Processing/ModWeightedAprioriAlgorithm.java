/**
 * ModWeightedAprioriAlgorithm class.
 * 
...
public class ModWeightedAprioriAlgorithm{
	...
	private ArrayList candidates, frequent, fCounters, fSupport, fWeights, fTransList;
	
	private int[] cCounters, cLevel;
	
	private float[] cSupports, cWeights;
	...
	/**
	 * ModWeightedAprioriAlgorithm method.
	 * Constructor.
	 ...
	 */
	public ModWeightedAprioriAlgorithm(BuildFrequentItemsets owner){
		bfi = owner;
...
//		printFrequentItemsets();
	...
	final private void prune(TransRecords tr, float minSup, byte itemsetSize){
		...
		cWeights = new float[c1];
		cTransList = new Object[c1];
		for (int i = 0; i < c1; i++){
			cCounters[i] = 0;
			cWeights[i] = (float)0.0;
		}
		...
		for (int i = 0; i < t1; i++){
			...
			for (int j = 0; j < c1; j++){
				...
				for (int k = 0; k < c2; k++){
					...
					if (i1 != -1 && (trans[i1].equals("0")) || trans[i1].equals("?")){
						incCount = false;
						break;
					}
					else{
						try{
							if (Float.valueOf(trans[i1]).floatValue() < minWeight){
								minWeight = Float.valueOf(trans[i1]).floatValue();
							}
						}
				...
				if (incCount){
					...
					cWeights[j] = cWeights[j] + minWeight;
				}
				incCount = true;
			}
		}
		...
		if (candidates.size() > 0){
			cl.addEntry(candidates, cCounters, cWeights, cSupports, cTransList);
		}
		if (frequent.size() > 0){
			fl.addEntry(frequent, fCounters, fWeights, fSupport, fTransList);
		}
	}
