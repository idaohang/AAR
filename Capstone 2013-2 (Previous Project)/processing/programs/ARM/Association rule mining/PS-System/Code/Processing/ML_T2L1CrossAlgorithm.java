...
public class ML_T2L1CrossAlgorithm{
	 ...
	 */
	public ML_T2L1CrossAlgorithm(BuildFrequentItemsets owner){
		bfi = owner;
	...
	final public void runAlgorithm(TransRecords tr, float minCon, float minSup){
		deriveNumberOfLevels(tr.getNames());
...
//			filterTransTable();
	...
	final private void deriveNumberOfLevels(String[] topicNames){
		int size = topicNames.length;
		int level = 0;
		for (int i = 0; i < size; i++){
			level = topicNames[i].split("-").length;
